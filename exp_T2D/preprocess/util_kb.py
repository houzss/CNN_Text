...
def query_super_classes(cls):
    ...
    s.setQuery(statement)
    s.setReturnFormat(JSON)
    results = s.query().convert()
    for result in results['results']['bindings']:
        cls = result['superclass']
        cls_uri = cls['value']
        supers.add(cls_uri.split(dbo_prefix)[1])
    # result = s.query(statement)
    # for row in result.fetchone():
    #     super_str = str(row[0])
...
def query_abstract_of_entity(ent):
    text = []
    try:
        # s = sparql.Service('http://dbpedia.org/sparql', "utf-8", "GET")
        s = SPARQLWrapper('http://dbpedia.org/sparql')
        statement = 'select distinct ?e where { <%s%s> dbo:abstract ?e.}' % (dbp_prefix, ent)
        s.setQuery(statement)
        s.setReturnFormat(JSON)
        results = s.query().convert()
        for result in results['results']['bindings']:
            #print(result["label"]["value"])
            if result['e']['xml:lang'] == 'en':
            # cls = result['superclass']
            # cls_uri = cls['value']
                text.append(result['e']['value'])
